...
func InsertDeviceData(deviceData DeviceData) error {
	...
	_, err := utils.DB.Exec(`
		...
		deviceData.DeviceID, deviceData.Latitude, deviceData.Longitude, deviceData.DeviceCount, deviceData.Timestamp)
	return err
}
...
func GetDeviceDataHandler(c *fiber.Ctx) error {
	...
	rows, err := utils.DB.Query("SELECT id, device_id, latitude, longitude, device_count, timestamp FROM device_data")
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&device.ID, &device.DeviceID, &device.Latitude, &device.Longitude, &device.DeviceCount, &device.Timestamp); err != nil {
